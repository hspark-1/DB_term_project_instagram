...
public interface DMRepository extends JpaRepository<DM, Long> {
	...
	@Query(value =
            "SELECT * FROM DM s WHERE (s.sender_id = ?1 AND s.receiver_id = ?2) OR (s.receiver_id = ?1 AND s.sender_id = ?2) ORDER BY time asc",
            nativeQuery = true)
	List<DM> findBySenderId_ReceiverId(String senderId, String receiverId);

	@Query(value =
            "SELECT * FROM DM s WHERE s.receiver_id = ?1 GROUP BY sender_id ORDER BY time asc",
            nativeQuery = true)
